...
class LotTest
  ...
  it should "transition through valid states correctly" in {
    ...
    val callchain2 = List(CreateLot(probe.ref, "0", "test2", "test")) ++ List(LotState.IN_PREVIEW, LotState.OPEN, LotState.FINISHED).map(SetLotState(probe.ref, _))
    ...
    val callchain3 = List(CreateLot(probe.ref, "0", "test3", "test")) ++ List(LotState.IN_PREVIEW, LotState.CLOSED).map(SetLotState(probe.ref, _))
